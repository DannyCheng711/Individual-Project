...
class MultiViewYoloDetection(nn.Module):
    """Detection head similar to McuYolo but takes fused features"""
    def __init__(self, num_classes=20, num_anchors=5, final_ch=320, passthrough_ch=96, mid_ch=512, s2d_r=2):
        ...
        self.space_to_depth = SpaceToDepth(s2d_r)
    ...
    def forward(self, final_feat, passthrough_feat):
        x = self.conv1(final_feat)
        x = self.conv2(x)
...
class McuYoloBackbone(nn.Module):
...
class MultiViewYolo(nn.Module):
    def __init__(self, num_classes=20, num_anchors=5, final_ch=320, passthrough_ch=96, mid_ch=512, s2d_r=2):
        super().__init__()
        # Two identical backbones
        self.backbone_a = McuYoloBackbone()
        self.backbone_b = McuYoloBackbone()
        ...
        # Detection head
        self.detection_head = MultiViewYoloDetection(
            num_classes=num_classes,
            num_anchors=num_anchors,
            final_ch=final_ch,
            passthrough_ch=passthrough_ch,
            mid_ch=mid_ch,
            s2d_r=s2d_r
        )
    ...
    def forward(self, inputs_a, inputs_b):
        ...
        passthrough_a, final_a = self.backbone_a(inputs_a)
        passthrough_b, final_b = self.backbone_b(inputs_b)
        
        # Fuse features using learnable alignment blocks
        fused_final = self.final_alignment(final_a, final_b)
        fused_passthrough = self.passthrough_alignment(passthrough_a, passthrough_b)
        
        # Pass fused features to detection head
        return self.detection_head(fused_final, fused_passthrough)
